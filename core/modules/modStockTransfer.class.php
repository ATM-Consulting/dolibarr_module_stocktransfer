...
class modStockTransfer extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->numero = 104932; // TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module
		...
		$this->description = "StockTransferDescription";
		...
		$this->config_page_url = array("setup.php@stocktransfer");
		// Dependencies
		...
		*/
		...
		$this->rights[$r][1] = 'Read objects of StockTransfer'; // Permission label
		...
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Create/Update objects of StockTransfer'; // Permission label
		$this->rights[$r][4] = 'stocktransfer'; // In php code, permission will be checked by test if ($user->rights->stocktransfer->level1->level2)
		$this->rights[$r][5] = 'write'; // In php code, permission will be checked by test if ($user->rights->stocktransfer->level1->level2)
		$r++;
		$this->rights[$r][0] = $this->numero + $r; // Permission id (must not be already used)
		$this->rights[$r][1] = 'Delete objects of StockTransfer'; // Permission label
		$this->rights[$r][4] = 'stocktransfer'; // In php code, permission will be checked by test if ($user->rights->stocktransfer->level1->level2)
		...
		*/
		$this->menu[$r++]=array(
			...
			'type'=>'left',
			'titre'=>$langs->trans('ModuleStockTransferName'),
			'mainmenu'=>'products',
		...
		$this->menu[$r++]=array(
			// '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'fk_menu'=>'fk_mainmenu=products,fk_leftmenu=stocktransfer_stocktransfer',
			// This is a Left menu entry
			'type'=>'left',
			'titre'=>$langs->trans('StockTransferNew'),
			'mainmenu'=>'products',
			'leftmenu'=>'stocktransfer_stocktransfernew',
			'url'=>'/stocktransfer/stocktransfer_card.php?action=create',
			// Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'langs'=>'stocktransfer@stocktransfer',
			'position'=>1100+$r,
